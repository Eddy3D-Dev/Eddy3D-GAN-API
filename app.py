"""
Eddy3D GAN Wind Prediction — Gradio + ZeroGPU

Converts the ONNX generator to PyTorch at startup, then runs inference
on a ZeroGPU-allocated H200 via the @spaces.GPU decorator.

API callers (e.g. Grasshopper/C#) can POST to /api/predict or use the
Gradio Python/JS client.
"""
...
def _build_colormap_lut() -> np.ndarray:
    turbo_f32 = (TURBO_COLORMAP * 255.0).astype(np.float32)
    half_l2 = 0.5 * np.sum(turbo_f32 ** 2, axis=1)

    lut = np.empty((256, 256, 256), dtype=np.uint8)
    gb = np.mgrid[0:256, 0:256].astype(np.float32).reshape(2, -1).T

    for r in range(256):
        pixels = np.empty((65536, 3), dtype=np.float32)
        pixels[:, 0] = r
        pixels[:, 1:] = gb
        scores = pixels @ turbo_f32.T
        scores -= half_l2
...
logger.info("Building colormap LUT ...")
COLORMAP_LUT = _build_colormap_lut()
logger.info("Colormap LUT ready (%.1f MB)", COLORMAP_LUT.nbytes / 1e6)
...
# ---------------------------------------------------------------------------
# Model loading — convert ONNX to PyTorch once at startup
# ---------------------------------------------------------------------------
...
def _load_pytorch_model() -> torch.nn.Module:
...
# ---------------------------------------------------------------------------
# Inference (GPU-accelerated via ZeroGPU)
# ---------------------------------------------------------------------------
@spaces.GPU
def _run_inference(data_b64: str) -> dict:
    """Full inference pipeline: decode -> GPU forward pass -> encode results.

    Decorated with @spaces.GPU so ZeroGPU allocates an H200 for this call.
    """
    ...
    if arr.size != expected:
        raise gr.Error(f"Expected {expected} floats (3*512*512), got {arr.size}.")

    input_tensor = torch.from_numpy(arr.reshape(1, 3, 512, 512)).cuda()

    # Move model to GPU and run
    model.cuda()
    with torch.no_grad():
        output = model(input_tensor)

    # Back to CPU numpy
    raw_output = output[0].cpu().numpy()  # (3, 512, 512)

    # Denormalize to uint8 image
    denorm = (raw_output.transpose(1, 2, 0) + 1.0) * 127.5
    denorm = np.clip(denorm, 0, 255).astype(np.uint8)  # (512, 512, 3)

    # Wind speeds via LUT
    wind_speeds_arr = _color_to_windspeed(denorm)
    compressed_ws = gzip.compress(wind_speeds_arr.tobytes(), compresslevel=1)
    wind_speeds_b64 = base64.b64encode(compressed_ws).decode("ascii")

    # PNG image
    buf = io.BytesIO()
    Image.fromarray(denorm, "RGB").save(buf, format="PNG", compress_level=1)
...
demo = gr.Interface(
    fn=_run_inference,
    inputs=gr.Textbox(label="data_b64", lines=2,
                      placeholder="Base64-encoded, gzip-compressed float32 array "
                                  "(3*512*512 values, channel-first, normalised to [-1,1])"),
    outputs=gr.JSON(label="Result"),
    api_name="predict",
    title="Eddy3D GAN Wind Prediction",
    description="Urban pedestrian-level wind flow prediction via GAN surrogate model. "
                "Submit input programmatically via the API.",
)
